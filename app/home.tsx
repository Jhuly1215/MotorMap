...
export default function HomeScreen() {
  ...
  // Estadísticas reales a partir de los niveles completados
  type LevelStat = { accuracy: number; unlocked: boolean };
  const completedEntries: [string, LevelStat][] = (
    Object.entries(state.completedLevels) as [string, LevelStat][]
  ).filter(([, v]) => v && v.accuracy > 0);
  const levelsDone = completedEntries.length;
  const totalLevels = (Object.values(LEVELS) as Level[][])
    .reduce((acc: number, arr: Level[]) => acc + arr.length, 0);
  const stars = completedEntries.reduce(
    (acc: number, [, v]) => acc + starsForAccuracy(v.accuracy), 0);
  const avgAccuracy = levelsDone > 0
    ? Math.round(completedEntries.reduce((acc: number, [, v]) => acc + v.accuracy, 0) / levelsDone)
    : 0;
      ...
      <Card style={styles.progressCard} variant="elevated">
        ...
        <View style={styles.statsRow}>
          <View style={styles.stat}>
            <Text style={styles.statValue}>{levelsDone}<Text style={styles.statTotal}>/{totalLevels}</Text></Text>
          ...
          <View style={styles.statDivider} />
          <View style={styles.stat}>
            <Text style={styles.statValue}>{levelsDone > 0 ? avgAccuracy + '%' : '—'}</Text>
